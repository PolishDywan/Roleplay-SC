...
public class CharacterCreationModule : ITransientScript
{
    ...
    {
        if (player == null || characterModel == null)
        {
            _logger.LogError("Player or CharacterModel is null.");
            return;
        }
        ...
        _logger.LogInformation($"Starting OpenAsync with player: {player?.Name}, CharacterModel: {characterModel?.Id}");
