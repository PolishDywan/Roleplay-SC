...
public class AuthenticationModule : ISingletonScript
{
    ...
    private async Task<bool> CheckMaintenanceStatus(ServerPlayer player)
    {
        ...
        // Bypassing the whitelist check by always setting canJoin to true
        var canJoin = true; // _discordModule.CanJoin(player.DiscordId);

        // Since we're now allowing all players to join, the logging and kicking for not being on the whitelist are no longer needed.
        // However, for educational purposes or future use, let's keep it commented so you know what was being done.
        /*
        if (!canJoin)
        {
