...
export class CharacterHandler {
    ...
    constructor(private readonly character: CharacterModule, private readonly player: Player, private readonly camera: CameraModule, private readonly event: EventModule, private readonly gui: GuiModule, private readonly weather: WeatherModule, private readonly house: HouseModule, private readonly loading: LoadingSpinnerModule, private readonly logger: LoggerModule, private readonly phone: PhoneModule) {
        this.logger.log('CharacterHandler initialized');

    }

    @on("keydown")
    public onKeydown(key: number): void {
        this.logger.log(`Key down: ${key}`); // Debug: Logging key press

        if (this.player.isInAPrison || !this.player.isSpawnedCharacter || !this.player.isControlsEnabled) {
            this.logger.log('Keydown ignored due to player state'); // Debug: Logging reason for ignoring keydown
            return;
        }

        if (key === KeyCodes.F9) {
            this.logger.log(`F9 pressed, isMenuOpen: ${this.isMenuOpen}, getIsAnyMenuOpen: ${this.player.getIsAnyMenuOpen}`); // Debug: Logging menu state
    ...
    public updateModel(character: CharacterInterface): void {
        this.logger.log(`Updating character model with ID: ${character.id}`); // Debug: Logging character application
    ...
    public spawn(character: CharacterInterface): void {
        ...
        this.logger.log(`Spawning character with ID: ${character.id}`); // Debug: Logging character spawn
