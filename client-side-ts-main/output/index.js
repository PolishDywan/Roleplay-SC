...
let GuiModule = class GuiModule {
    ...
    webview;
    webViewReadyPromise;
    resolveWebViewReady; // Ensures initialization.
    eventQueue = []; // Queue for event registrations.
...
SessionHandler = __decorate([
    foundation(),
    singleton(),
    __metadata("design:paramtypes", [Player, CameraModule, GuiModule, LoadingSpinnerModule, LoggerModule])
], SessionHandler);
...
(function (NotificationTypes) {
    ...
    NotificationTypes[NotificationTypes["WARNING"] = 1] = "WARNING";
    NotificationTypes[NotificationTypes["ERROR"] = 2] = "ERROR";
    NotificationTypes[NotificationTypes["SUCCESS"] = 3] = "SUCCESS";
...
let VehicleModule = class VehicleModule {
    ...
    getClosestVehicleDoor(vehicle) {
        ...
        for (let i = 0, l = native.getNumberOfVehicleDoors(_vehicle); i < l; i++) {
            const distance = this.math.distance(alt.Player.local.pos, native.getEntryPointPosition(_vehicle, i));
            if (obj.distance == null || obj.distance > distance) {
                obj.distance = distance;
                obj.door = i;
            }
...
VehicleSelectorHandler = __decorate([
    foundation(),
    singleton(),
    __metadata("design:paramtypes", [CameraModule, EventModule, UpdateModule, VehicleModule, LoggerModule, NotificationModule, CharCreatorModule])
...
let WebviewHandler = class WebviewHandler {
    ...
    create(url) {
        console.log(`Tworzenie widoku GUI z URL: ${url}`);
...
(function (LicenseType) {
    ...
    LicenseType[LicenseType["TOWING_AGENCY"] = 8] = "TOWING_AGENCY";
    LicenseType[LicenseType["VEHICLE_DEALERSHIP"] = 16] = "VEHICLE_DEALERSHIP";
...
let WeatherHandler = class WeatherHandler {
    ...
    onUpdateWeather(secondsForTransition) {
        ...
        this.interval = alt__default.setInterval(() => {
            percentage++;
            if (percentage < 100) {
                native__default.setCurrWeatherState(oldWeatherHash, currentWeatherHash, (percentage / 100));
            }
            else {
                alt__default.clearInterval(this.interval);
        ...
        if (weatherString === "XMAS") {
            native__default.useSnowWheelVfxWhenUnsheltered(true);
            native__default.useSnowFootVfxWhenUnsheltered(true);
        }
        else {
            native__default.useSnowWheelVfxWhenUnsheltered(false);
            native__default.useSnowFootVfxWhenUnsheltered(false);
        }
    }
};
__decorate([
...
var METAKEY_STREAM_SYNC;
(function (METAKEY_STREAM_SYNC) {
    METAKEY_STREAM_SYNC["DEATH_STATE"] = "deathState";
    METAKEY_STREAM_SYNC["SIREN_MUTED"] = "sirenMuted";
})(METAKEY_STREAM_SYNC || (METAKEY_STREAM_SYNC = {}));
    ...
    onKeydown(key) {
        if (key === KeyCodes.U) {
            if (this.isMenuOpen) {
                ...
                const deathState = alt__default.Player.local.getStreamSyncedMeta(METAKEY_STREAM_SYNC.DEATH_STATE);
...
let DeathHandler = class DeathHandler {
    ...
    async onStreamSyncedMetaChange(entity, key, value, oldValue) {
        ...
        alt__default.Player.local.getStreamSyncedMeta(METAKEY_STREAM_SYNC.DEATH_STATE);
    }
    async onStart() {
        native__default.setPedCanBeTargetted(alt__default.Player.local.scriptID, false);
        native__default.setEntityInvincible(alt__default.Player.local.scriptID, true);
        native__default.setPedCanRagdoll(alt__default.Player.local.scriptID, true);
        alt__default.setTimeout(() => {
            native__default.freezeEntityPosition(alt__default.Player.local.scriptID, true);
        }, 1000);
    }
    async onRevive() {
        native__default.setPedCanBeTargetted(alt__default.Player.local.scriptID, true);
        native__default.setEntityInvincible(alt__default.Player.local.scriptID, false);
        native__default.setPedCanRagdoll(alt__default.Player.local.scriptID, true);
        alt__default.setTimeout(() => {
            native__default.freezeEntityPosition(alt__default.Player.local.scriptID, false);
        }, 1000);
    }
};
__decorate([
...
let VehicleSirenHandler = class VehicleSirenHandler {
    ...
    keydown(key) {
        if (!alt__default.Player.local.vehicle || this.player.getIsAnyMenuOpen) {
            return;
        }
        if (key === KeyCodes.G) {
...
let CharacterSelectorHandler = class CharacterSelectorHandler {
    ...
    async openCharSelector(characterDatas, lastSelectedCharacterId) {
        ...
        this.loading.show("Wczytywanie wyboru postaci...");
        alt.setTimeout(() => {
            this.player.fadeIn(500);
            this.player.unblurScreen(500);
            this.loading.hide();
...
let VehicleServiceHandler = class VehicleServiceHandler {
    ...
    onUpdateTuningPart(type, value) {
        ...
        if (type === VehicleModType.Colour1 || type === VehicleModType.Colour2) {
            native__default.setVehicleColours(this.showcaseVehicle, this.primColor, this.secColor);
        }
        native__default.setVehicleMod(this.showcaseVehicle, type, value, false);
    }
    onRotate(dir) {
    ...
    tick() {
        if (this.isRotating) {
            ...
            let heading = native__default.getEntityHeading(this.showcaseVehicle);
            const newHeading = (heading += this.dir);
            native__default.setEntityHeading(this.showcaseVehicle, newHeading);
        }
        const scriptID = alt__default.Player.local.scriptID;
        native__default.setEntityAlpha(scriptID, 0, false);
        native__default.setEntityCollision(scriptID, false, false);
        native__default.freezeEntityPosition(scriptID, true);
        native__default.setPedCanBeTargetted(scriptID, false);
        native__default.removeAllPedWeapons(scriptID, true);
        native__default.disableControlAction(0, InputType.NEXT_CAMERA, true);
        const rightClicked = native__default.getDisabledControlNormal(0, InputType.AIM);
        if (rightClicked) {
            this.player.hideCursor();
        }
        else {
    ...
    async createVehicle(data) {
        ...
        const hash = alt__default.hash(data.vehicleModelName);
        await loadModel(hash);
        const pos = alt__default.Player.local.pos;
        this.showcaseVehicle = this.vehicle.createShowcaseVehicle(hash, pos.x, pos.y, pos.z, 0, data.primaryColor, data.secondaryColor);
        native__default.networkSetInSpectatorMode(true, this.showcaseVehicle);
    }
    close() {
        this.camera.destroyCamera();
        this.player.isSpawnedCharacter = true;
        ...
        native__default.resetEntityAlpha(alt__default.Player.local.scriptID);
        native__default.setEntityCollision(alt__default.Player.local.scriptID, true, true);
        native__default.freezeEntityPosition(alt__default.Player.local.scriptID, false);
        this.event.emitGui("gui:routeto", "game");
        this.gui.unfocusView();
        native__default.deleteVehicle(this.showcaseVehicle);
