...
@PrototypeFor(alt.Player) @singleton()
export class Player extends alt.Player {
    ...
    public constructor(private readonly update: UpdateModule, private readonly event: EventModule, private readonly logger: LoggerModule) {
        super();
    ...
    public updatePositionInHUD(force: boolean = false): void {
        const getStreet = native.getStreetNameAtCoord(alt.Player.local.pos.x, alt.Player.local.pos.y,
                alt.Player.local.pos.z, 0, 0);
        ...
        const zone = native.getFilenameForAudioConversation(
