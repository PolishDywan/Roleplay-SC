...
export default class CharCreator extends Vue {
    ...
    @Ref() public readonly formularMenu!: CharacterFormular;
    @Ref() public readonly genderMenu!: Gender;
    @Ref() public readonly parentsMenu!: Parents;
    @Ref() public readonly faceFeaturesMenu!: FaceFeatures;
    @Ref() public readonly characterAppearanceMenu!: CharacterAppearance;
    @Ref() public readonly clothesMenu!: ClothesMenu;
    @Ref() public readonly tattoosMenu!: Tattoos;

    public purchaseOrders: CharacterCreatorPurchaseInterface[] = [];
    public characterCosts = 0;

    public blockAllNavButtons = false;
    public isNewCharacter = true;
    public isSaving = false;
    public tabIndex = 0;
    public resetCameraButtonVisible = false;
    public cameraButtonsGotClicked = false;
    ...
    public onSetCharacter(character: CharacterInterface, maxDrawables: MaxDrawablesInterface, isNewCharacter: boolean): void {
    ...
    public onSetGender(gender: number, maxDrawables: MaxDrawablesInterface): void {
    ...
    public onUpdatePurchaseOrders(purchaseOrders: CharacterCreatorPurchaseInterface[]): void {
    ...
    public removePurchaseOrder(purchaseOrder: CharacterCreatorPurchaseInterface): void {
    ...
    public requestBuyCharacter(): void {
    const characterData = characterCreationService.getCharacterCreatorData;
    ...
    // Następnie sprawdzaj właściwości character przed uzyskaniem do nich dostępu
    if (character.firstName === "") {
        this.formularMenu.firstNameValidation = "EMPTY";
    } else {
        if (character.firstName.length <= 1) {
            this.formularMenu.firstNameValidation = "TO_SHORT";
        } else if (!/^[a-zA-ZÀ-ž]+$/.test(character.firstName)) {
    ...
    public changeCameraPos(posIndex: number): void {
    ...
    public rotateCharacter(dir: number): void {
        alt.emit("charcreator:rotatecharacter", dir);
    }

    public stopRotateCharacter(): void {
        alt.emit("charcreator:rotatestop");
    }

    public closeCharCreation(): void {
